# The Polycube Model
A computational model of polycube self-assembly, in which cubes connect via specified interaction rules to form polycubes. The model also applies to 2D polyomino assembly.
...
Each tile has six sides by default. To switch to 2D mode of assembly, the last two sides are considered '00' (neutral). This refers to 'back' and 'front' faces of the cube, essentially reducing to two dimensional assembly. Orientations do not matter in 2D assembly, hence a default value is used. However the output shapes are returned in [(x,y,z)] format with z=0 for all tuples.
